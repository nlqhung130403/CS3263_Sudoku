...
def get_box(row, col, box_size):
    """
    Get the box number for a given cell position.
    
    Args:
        row: Row index (0 to grid_size-1)
        col: Column index (0 to grid_size-1)
        box_size: Size of each box (e.g., 3 for 9×9, 4 for 16×16)
    
    Returns:
        Box number (0 to (grid_size/box_size)^2 - 1)
    """
...
def create_sudoku_csp(puzzle_string):
    ...
    for i in range(grid_size):
        for j in range(grid_size):
            ...
            if char is not None:
                ...
                domains[(i, j)] = list(range(1, grid_size + 1))
    
    # Neighbors: cells in the same row, column, or box are neighbors
    ...
    for i in range(grid_size):
        for j in range(grid_size):
            ...
            box_row_start = (i // box_size) * box_size
            box_col_start = (j // box_size) * box_size
            for box_row in range(box_row_start, box_row_start + box_size):
                for box_col in range(box_col_start, box_col_start + box_size):
...
def solve_sudoku(puzzle_string, inference_method):
    ...
    return solve_sudoku_csp(csp, inference_method)
