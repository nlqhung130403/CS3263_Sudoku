...
\subsection{CSP Formulation for Sudoku}
...
In Sudoku, we model the constraints using binary ``not equal'' constraints. Each cell must differ from its 20 peers (cells in the same row, column, or $3 \times 3$ box). This results in approximately $81 \times 20 / 2 = 810$ undirected edges, or $1{,}620$ directed arcs in the constraint graph. The constraint function $C(X_i, v_i, X_j, v_j)$ returns \texttt{True} if and only if $v_i \neq v_j$ when $X_i$ and $X_j$ are neighbors.
...
\Function{FORWARD-CHECKING}{csp, $var$, $value$, assignment, removals}
    \For{each unassigned neighbor $B$ of $var$}
...
\begin{algorithm}[htbp]
\caption{Maintaining Arc Consistency (MAC)}
\label{alg:mac}
\begin{algorithmic}[1]
\Function{MAC}{csp, $var$, $value$, assignment, removals}
    \State $queue \gets \{(X, var) \mid X \in \Call{NEIGHBORS}{csp, $var$}\}$
    \State \Return \Call{AC-3B}{csp, $queue$, removals}
\EndFunction
\Statex
\Function{AC-3B}{csp, $queue$, removals}
    \While{$queue$ is not empty}
        \State $(X_i, X_j) \gets queue.pop()$
        \State $revised \gets$ \Call{REVISE}{csp, $X_i$, $X_j$, removals}
        \If{$revised$}
            \If{\Call{CURR-DOMAINS}{csp, $X_i$} is empty}
                \State \Return false
            \EndIf
            \For{each neighbor $X_k$ of $X_i$ where $X_k \neq X_j$}
                \State $queue.add((X_k, X_i))$
            \EndFor
        \EndIf
    \EndWhile
    \State \Return true
